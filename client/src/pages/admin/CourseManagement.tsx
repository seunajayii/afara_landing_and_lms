      ...
      <Dialog open={Boolean(lessonDialog)} onOpenChange={(open) => !open && setLessonDialog(null)}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4">
           ...
           <DialogFooter><Button variant="outline" onClick={() => setLessonDialog(null)}>Cancel</Button><Button disabled={!lessonDraft.title.trim() || lessonMutation.isPending || (lessonDraft.lessonType === "live_session" && !lessonDraft.eventStartTime)} onClick={() => lessonMutation.mutate()}>{lessonMutation.isPending ? "Saving…" : "Save lesson"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
