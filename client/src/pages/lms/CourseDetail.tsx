...
import { ArrowLeft, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Circle, Download, ExternalLink, FileText, Lock, PlayCircle, ShieldCheck, Users, Video } from "lucide-react";
import type { Course, Event, Lesson, LessonProgress, Module, Resource } from "@shared/schema";

type EventWithFacilitators = Event & { facilitators?: Array<{ id: string; firstName: string; lastName: string }> };
type LessonWithResource = Lesson & { resource?: Resource | null; event?: EventWithFacilitators | null; contentAvailable?: boolean };
            ...
            <div>
              ...
              {allLessons.length === 0 ? <Card className="py-12 text-center"><CardContent><FileText className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><h2 className="text-lg font-semibold">Content is being prepared</h2><p className="mt-1 text-sm text-muted-foreground">Check back when the course lessons are published.</p></CardContent></Card> :
                <div className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
                  <aside className="rounded-lg border bg-card lg:sticky lg:top-4 lg:max-h-[calc(100vh-4rem)] lg:overflow-y-auto"><div className="border-b p-4"><h2 className="font-semibold">Course content</h2><p className="mt-1 text-xs text-muted-foreground">Pick up where you left off at any time.</p></div>{courseQuery.data.modules.map((module, index) => <div key={module.id} className="border-b last:border-b-0"><p className="px-4 pt-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Module {index + 1}: {module.title}</p><div className="p-2">{module.lessons.map((lesson) => <button key={lesson.id} type="button" onClick={() => selectLesson(lesson)} className={`flex w-full items-start gap-2 rounded-md p-2.5 text-left text-sm transition-colors ${activeLesson?.id === lesson.id ? "bg-primary/10 text-primary" : "hover:bg-muted"}`} data-testid={`lesson-nav-${lesson.id}`}>{lesson.lessonType === "live_session" ? <CalendarDays className="mt-0.5 h-4 w-4 shrink-0" /> : completedIds.has(lesson.id) ? <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /> : activeLesson?.id === lesson.id ? <PlayCircle className="h-4 w-4 shrink-0" /> : <Circle className="h-4 w-4 shrink-0 text-muted-foreground" />}<span className="min-w-0 flex-1"><span className="block truncate">{lesson.title}</span>{lesson.lessonType === "live_session" && lesson.event && <span className="mt-0.5 block text-xs text-muted-foreground">{new Date(lesson.event.startTime).toLocaleDateString(undefined, { month: "short", day: "numeric" })} · {new Date(lesson.event.startTime).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}</span>}</span></button>)}</div></div>)}</aside>
                  {activeLesson && <section className="min-w-0"><Card><CardContent className="p-5 md:p-7"><div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><div className="mb-2 flex gap-2"><Badge variant="outline">{activeLesson.lessonType === "downloadable" ? "Download" : activeLesson.lessonType}</Badge>{completedIds.has(activeLesson.id) && <Badge className="bg-emerald-500/10 text-emerald-700">Completed</Badge>}</div><h2 className="text-2xl font-bold">{activeLesson.title}</h2>{activeLesson.description && <p className="mt-2 text-muted-foreground">{activeLesson.description}</p>}</div><span className="text-sm text-muted-foreground">{formatDuration(activeLesson.durationMinutes)}</span></div>
                      {activeLesson.lessonType === "live_session" && activeLesson.event ? <div className="overflow-hidden rounded-xl border"><div className="bg-primary/10 p-6"><div className="mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-primary text-primary-foreground"><CalendarDays className="h-5 w-5" /></div><p className="text-sm font-medium text-primary">Scheduled live class</p><h3 className="mt-1 text-xl font-semibold">{formatClassDate(activeLesson.event.startTime)}</h3>{activeLesson.event.endTime && <p className="mt-1 text-sm text-muted-foreground">Ends {new Date(activeLesson.event.endTime).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}</p>}</div><div className="space-y-4 p-6"><div className="flex items-start gap-3"><Users className="mt-0.5 h-5 w-5 text-muted-foreground" /><div><p className="text-sm font-medium">{(activeLesson.event.facilitators?.length || 0) > 1 ? "Facilitators" : "Facilitator"}</p><p className="text-sm text-muted-foreground">{activeLesson.event.facilitators?.map((facilitator) => `${facilitator.firstName} ${facilitator.lastName}`).join(", ") || "To be announced"}</p></div></div>{activeLesson.event.meetingPlatform && <p className="text-sm text-muted-foreground">Hosted on {activeLesson.event.meetingPlatform}</p>}<div className="flex flex-wrap gap-2">{activeLesson.event.meetingLink && <Button asChild><a href={activeLesson.event.meetingLink} target="_blank" rel="noreferrer">Join live class<ExternalLink className="ml-2 h-4 w-4" /></a></Button>}<CalendarActions event={activeLesson.event} eventPageUrl={`${window.location.origin}/lms/courses/${id}`} testId="button-add-live-class-to-calendar" /></div></div></div> :
