            ...
            {!isLoading && cohorts && cohorts.length > 0 && (
              <div className="grid md:grid-cols-2 gap-8">
                {cohorts.map((cohort) => {
                  ...
                  return (
                    <Card key={cohort.id} className="flex flex-col hover-elevate transition-all duration-300" data-testid={`card-cohort-${cohort.slug}`}>
                      {cohort.heroImageUrl && (
                        <div className="h-44 w-full overflow-hidden rounded-t-md">
                          <img src={cohort.heroImageUrl} alt={cohort.displayName || cohort.name} className="w-full h-full object-cover" />
                        </div>
                      )}
                      <CardHeader>
